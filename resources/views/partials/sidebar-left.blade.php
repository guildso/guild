<nav x-data="{ open: false }" class="relative top-0 left-0 z-10 flex flex-col items-start justify-between w-48 flex-shrink-0 h-screen border-r border-gray-100 dark:border-gray-800">
    ...
    <div class="relative flex flex-col">
        ...
        <div class="flex flex-col items-start w-full pl-5 mt-10 space-y-2">
                ...
                <a href="{{ route('dashboard') }}" class="@if(request()->routeIs('dashboard')){{ 'text-gray-800 dark:text-gray-100' }}@else{{ 'text-gray-400' }}@endif flex items-center pl-1 text-base font-semibold">
                    🏠 <span class="pl-2 text-sm">Home</span>
                </a>

                <a href="{{ route('shifts') }}" class="@if(request()->routeIs('shifts')){{ 'text-gray-800 dark:text-gray-100' }}@else{{ 'text-gray-400' }}@endif flex items-center pl-1 text-base font-semibold">
                    🕗 <span class="pl-2 text-sm">My Shift</span>
                </a>

                <a href="{{ route('tasks') }}" class="@if(request()->routeIs('tasks')){{ 'text-gray-800 dark:text-gray-100' }}@else{{ 'text-gray-400' }}@endif flex items-center pl-1 text-base font-semibold">
                    ✅ <span class="pl-2 text-sm">Tasks</span>
                </a>

                <a href="{{ route('badges') }}" class="@if(request()->routeIs('badges')){{ 'text-gray-800 dark:text-gray-100' }}@else{{ 'text-gray-400' }}@endif flex items-center pl-1 text-base font-semibold">
                    🥇 <span class="pl-2 text-sm">Badges</span>
                </a>

                <a href="{{ route('airdrop') }}" class="@if(request()->routeIs('airdrop')){{ 'text-gray-800 dark:text-gray-100' }}@else{{ 'text-gray-400' }}@endif flex items-center pl-1 text-base font-semibold">
                    💸 <span class="pl-2 text-sm">AirDrop</span>
                </a>

                @if(auth()->user()->hasTeamRole(auth()->user()->currentTeam, 'manager'))
                    <a href="{{ route('news') }}" class="@if(request()->routeIs('news')){{ 'text-gray-800 dark:text-gray-100' }}@else{{ 'text-gray-400' }}@endif flex items-center pl-1 text-base font-semibold">
    ...
    <div class="relative w-full">
        ...
        <!-- Shift Start/Stop -->
        <div class="relative w-full pr-3 pb-3">
            @livewire('shifts')
        </div>

        @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
            <button @click="open=!open" class="flex items-center justify-between w-full px-5 py-3 text-sm text-gray-400 transition duration-150 ease-in-out border-t border-gray-100 dark:border-gray-700 hover:text-gray-600 dark:hover:text-gray-200 focus:outline-none hover:bg-gray-50 dark:hover:bg-gray-700">
        ...
        <div x-show="open" class="fixed bottom-0 left-0 w-64 h-auto pl-1 ml-64 overflow-hidden origin-bottom-left"
                ...
                <div class="py-1 bg-white dark:bg-gray-700 rounded-md shadow-xs">
                    <!-- Account Management -->
